...
如果同组结算单合并后的本期实销/销售额为 0 或负数，或文件体现退货、冲减、费用、手续费、快递费、负开票数量等，不要仅因 ERP sales_total 相等就输出 matched=true；必须在 issues 中说明这是负数/零销售调整场景，并列出 sales_total、net_sales_total、含税结账金额和最终应付款等差异证据，交人工复核。
