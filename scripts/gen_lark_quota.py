...
fig.suptitle("飞书 API 月额度分析（标准版）", fontsize=15, color='#1D39C4', fontweight='bold', y=0.98)
...
bars = ax1.bar(categories, per_task, color=['#3370FF', '#3370FF', '#FB8C00', '#E53935', '#8E24AA'],
...
ax1.text(2.0, -1.4, f"100 任务/月 → 约 {total_month} 次调用",
         ha='center', fontsize=11, color='#1D39C4', fontweight='bold')
...
# 中心文字
ax2.text(0, 0.12, f"{pct:.0f}%", fontsize=24, color='#1D39C4', fontweight='bold', ha='center')
ax2.text(0, -0.18, f"已用 {used} / {quota} 次", fontsize=11, color='#555555', ha='center')
ax2.text(0, -0.38, f"剩余 {remaining} 次", fontsize=11, color='#3370FF', ha='center')
...
plt.savefig('docs/lark_api_quota.png', dpi=150, bbox_inches='tight', facecolor='#FFFFFF')
plt.close()
