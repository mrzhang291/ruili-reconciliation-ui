...
test("routes reconciliation through the HTTP backend", async () => {
  ...
  assert.match(reconciliationService, /createReconciliationGroupTask/);
  assert.match(reconciliationService, /deterministic_batch_group/);
  ...
  assert.match(batchView, /批量结算单文件夹/);
  assert.match(batchView, /webkitdirectory/);
